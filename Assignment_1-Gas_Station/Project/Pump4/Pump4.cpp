...
CDataPool pumpDP("Pump4", sizeof(pumpStatus));
pumpStatus *pumpStatusPtr = (pumpStatus *)(pumpDP.LinkDataPool());

//**************************************************************************************************************************************************
CSemaphore prodPipe("pipe4Producer", 0, 1);
CSemaphore consPipe("pipe4Consumer", 1, 1);

CSemaphore prodPool("pool4Producer", 0, 1);
CSemaphore consPool("pool4Consumer", 1, 1);

//**************************************************************************************************************************************************
CPipe Pipe_in("Pipe4_in", PUMP_PIPE_SIZE);
...
int main(){
	...
	printf("::::: PUMP 4 :::::\tWaiting on Rendezvous...\n");
	r1.Wait();
	system("cls");
	while (endSimulation.Read() == 0){
		...
		*pumpStatusPtr = ps;
		...
		for (int i = 0; i < 4; i++){
			...
			while (!(tankPtr->currentFuelVolume> 200)){
				...
				TEXT_COLOUR(12);
				printf("::::: PUMP 4 :::::\tNot enough fuel on tank %d. Waiting for Refueling\n", (i + 1));
				SLEEP(5000);
				TEXT_COLOUR();
		...
		printf("::::: PUMP 4 :::::\tWaiting for another customer...\n");;
		//**************************************************************************************************************************************************
		...
		screenSemaphore.Wait();
		std::cout << "::::: PUMP 4 :::::\t" << "Customer " << c.customerName << " arrived\n";
		screenSemaphore.Signal();
		...
		//*****************************************************************************************************************************************
		screenSemaphore.Wait();
		printf("::::: PUMP 4 :::::\tCustomer selecting fuel...\n");
		screenSemaphore.Signal();
		...
		*pumpStatusPtr = ps;
		...
		screenSemaphore.Wait();
		printf("::::: PUMP 4 :::::\tCustomer selected fuel %d\n", selectedFuel);
		screenSemaphore.Signal();

		//*****************************************************************************************************************************************
		screenSemaphore.Wait();
		printf("::::: PUMP 4 :::::\tCustomer selecting fuel amount...\n");
		screenSemaphore.Signal();
		...
		*pumpStatusPtr = ps;
		...
		screenSemaphore.Wait();
		printf("::::: PUMP 4 :::::\tCustomer selected fuel amount = %d\n", selectedFuelAmount);
		screenSemaphore.Signal();
		...
		*pumpStatusPtr = ps;
		...
		// receives payment
		screenSemaphore.Wait();
		std::cout << "::::: PUMP 4 :::::\t" << "Waiting for payment\n";
		screenSemaphore.Signal();
		...
		*pumpStatusPtr = ps;
		...
		screenSemaphore.Wait();
		std::cout << "::::: PUMP 4 :::::\t" << "Hose removed from pump\n";
		screenSemaphore.Signal();
		SLEEP(3000); // wait certain time to put the hose on the car

		screenSemaphore.Wait();
		std::cout << "::::: PUMP 4 :::::\t" << "Dispensing fuel\n";
		screenSemaphore.Signal();
		...
		*pumpStatusPtr = ps;
		...
		screenSemaphore.Wait();
		std::cout << "::::: PUMP 4 :::::\t" << "Hose returned to pump\n";
		screenSemaphore.Signal();
