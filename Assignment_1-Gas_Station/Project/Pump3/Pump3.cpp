...
CDataPool pumpDP("Pump3", sizeof(pumpStatus));
pumpStatus *pumpStatusPtr = (pumpStatus *)(pumpDP.LinkDataPool());

//**************************************************************************************************************************************************
CSemaphore prodPipe("pipe3Producer", 0, 1);
CSemaphore consPipe("pipe3Consumer", 1, 1);

CSemaphore prodPool("pool3Producer", 0, 1);
CSemaphore consPool("pool3Consumer", 1, 1);

//**************************************************************************************************************************************************
CPipe Pipe_in("Pipe3_in", PUMP_PIPE_SIZE);
...
int main(){
	...
	printf("::::: PUMP 3 :::::\tWaiting on Rendezvous...\n");
	r1.Wait();
	system("cls");
	while (endSimulation.Read() == 0){
		...
		*pumpStatusPtr = ps;
		...
		for (int i = 0; i < 4; i++){
			...
			while (!(tankPtr->currentFuelVolume> 200)){
				...
				TEXT_COLOUR(12);
				printf("::::: PUMP 3 :::::\tNot enough fuel on tank %d. Waiting for Refueling\n", (i + 1));
				SLEEP(5000);
				TEXT_COLOUR();
		...
		printf("::::: PUMP 3 :::::\tWaiting for another customer...\n");;
		//**************************************************************************************************************************************************
		...
		screenSemaphore.Wait();
		std::cout << "::::: PUMP 3 :::::\t" << "Customer " << c.customerName << " arrived\n";
		screenSemaphore.Signal();
		...
		//*****************************************************************************************************************************************
		screenSemaphore.Wait();
		printf("::::: PUMP 3 :::::\tCustomer selecting fuel...\n");
		screenSemaphore.Signal();
		...
		*pumpStatusPtr = ps;
		...
		screenSemaphore.Wait();
		printf("::::: PUMP 3 :::::\tCustomer selected fuel %d\n", selectedFuel);
		screenSemaphore.Signal();

		//*****************************************************************************************************************************************
		screenSemaphore.Wait();
		printf("::::: PUMP 3 :::::\tCustomer selecting fuel amount...\n");
		screenSemaphore.Signal();
		...
		*pumpStatusPtr = ps;
		...
		screenSemaphore.Wait();
		printf("::::: PUMP 3 :::::\tCustomer selected fuel amount = %d\n", selectedFuelAmount);
		screenSemaphore.Signal();
		...
		*pumpStatusPtr = ps;
		...
		// receives payment
		screenSemaphore.Wait();
		std::cout << "::::: PUMP 3 :::::\t" << "Waiting for payment\n";
		screenSemaphore.Signal();
		...
		*pumpStatusPtr = ps;
		...
		screenSemaphore.Wait();
		std::cout << "::::: PUMP 3 :::::\t" << "Hose removed from pump\n";
		screenSemaphore.Signal();
		SLEEP(3000); // wait certain time to put the hose on the car

		screenSemaphore.Wait();
		std::cout << "::::: PUMP 3 :::::\t" << "Dispensing fuel\n";
		screenSemaphore.Signal();
		...
		*pumpStatusPtr = ps;
		...
		screenSemaphore.Wait();
		std::cout << "::::: PUMP 3 :::::\t" << "Hose returned to pump\n";
		screenSemaphore.Signal();
